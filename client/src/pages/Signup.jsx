...
export default function Signup() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", minHeight: '100vh', background: '#FDF6F9', display: 'flex' }}>
      ...
      {/* LEFT PANEL */}
      <div style={{ flex: 1, background: 'linear-gradient(160deg, #F4C0D1 0%, #C9B8E8 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '3rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', textAlign: 'center', color: 'white' }}>
          ...
          <p style={{ fontSize: '1.1rem', fontWeight: 300, lineHeight: 1.7, opacity: 0.9, maxWidth: 280 }}>
            Your safe space to understand your body — no shame, no taboo, just support 💕
          </p>
        ...
        <div style={{ width: '100%', maxWidth: 420 }}>
          <div style={{ marginBottom: '2rem' }}>
            ...
            <p style={{ color: '#7A5C6E', fontSize: '15px', fontWeight: 300 }}>
              Already have an account? <Link to="/login" style={{ color: '#D4537E', textDecoration: 'none', fontWeight: 500 }}>Login here</Link>
            </p>
          </div>
...
<div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
  <div>
    <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#2D1B2E', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Full Name</label>
    <input type="text" name="name" placeholder="Your name" value={form.name} onChange={handle}
  ...
  <div>
    ...
    <div style={{ position: 'relative' }}>
      <input type={showPassword ? 'text' : 'password'} name="password" placeholder="Min 6 characters" value={form.password} onChange={handle}
      ...
      <button onClick={() => setShowPassword(!showPassword)}
        style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '18px', color: '#7A5C6E', padding: 0 }}>
        {showPassword ? '✕' : '◉'}
      </button>
    </div>
  </div>
  ...
  <div>
    <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#2D1B2E', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>I am (optional)</label>
    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
      {[
        ...
        <button key={g.val} onClick={() => setForm({ ...form, gender: g.val })}
          style={{ border: `1.5px solid ${form.gender === g.val ? '#D4537E' : 'rgba(212,83,126,0.2)'}`, background: form.gender === g.val ? 'rgba(212,83,126,0.1)' : 'white', color: form.gender === g.val ? '#993556' : '#7A5C6E', padding: '8px 14px', borderRadius: '50px', cursor: 'pointer', fontSize: '13px', fontFamily: "'DM Sans', sans-serif", fontWeight: form.gender === g.val ? 500 : 400, transition: 'all 0.15s' }}>
          {g.label}
        </button>
      ))}
    </div>
